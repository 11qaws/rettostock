...
* **실시간 원격 반영 (옵트인)**: 기본은 외부 통신 없음. 고급 설정에서 켜면 브라우저(크롬 등)에서 바꾼 설정이 OBS 위젯에 바로 반영됩니다 — 변경은 전자서명되어 전달되므로 코드가 노출돼도 제3자가 위젯을 조작할 수 없습니다.
...
* **Opt-in live remote control**: zero external traffic by default. Enable it in advanced settings and changes made in your browser apply to the OBS widget instantly — every relayed change is cryptographically signed, so nobody else can drive your widget even if they see the code.
