...
## 2026-07-13: v1.0.48 Card shape and Detail-effect layering
- **Before -> after:** Theme Detail never changed geometry; any small difference between themes came from their pre-existing borders, shadows and content metrics. Advanced settings now provide **Card shape**: **Box** retains the original Amore Pink padding; **Card** trims List/Rotate padding and gaps just enough to read as a slightly wider card. Marquee remains unchanged because its 100px strip has its own compact sizing. New copied URLs carry `shape=card`; existing URLs with no shape parameter retain Box.
- **Detail glow fix:** the Card-level ±5/10/15% surge effect was still calculated, but a Detail theme's more-specific static `box-shadow` could cover its glow at the default Card effect level. The effect selector now has higher specificity and supplies both an outer and restrained inner glow. Dark Glass needed one extra scoped rule because its five `:not()` theme exclusions are more specific than the generic fix; its Detail surface can no longer hide the orange/ice direction glow. Full-mode motion, particles, record/target banners, price flash and all quote paths are unchanged.
- **Dark Glass tuning:** the selector was active, but its original 10% internal reflection had no visible outer edge against the dark base. Detail now adds a low-opacity violet rim and 20px outer halo, while keeping it below price-direction and event glows.
- **Local chart recovery:** the Cloudflare chart route allowed `127.0.0.1:4173` but omitted the equivalent `localhost:4173`. The local Vite preview therefore received a CORS 403 for every trend request while quote paths remained independent. An existing `ALLOWED_ORIGINS` Pages variable was also replacing the code defaults, so the resolver now merges its extra entries with the safe local/production list. No market-data cache or credential policy was widened.
- **Shape baseline and scrollbar cause:** Box retains its pre-existing Amore Pink geometry; only Card corrects Amore Pink's vertical padding to Aurora Starry Night's rendered size. The Amore `KORU` crowding was a secondary effect of an internal vertical scrollbar: `overflow-x: hidden` computed the root's y-axis as `auto`, and a tiny vertical overrun removed horizontal room from the 300px source. The root now hides both axes, so the `-131…` readout cannot lose width to a scrollbar.

## 2026-07-13: v1.0.47 Static theme detail
- **Before -> after:** Aurora Starry Night was the only theme with a clearly layered card surface (night base, sparse stars, aurora rim and coordinated light). New **Theme detail** in Advanced settings offers **Basic / Detail**. Detail brings the other themes up to the same restrained density: Amore gains a soft rose/lavender surface and inner highlight, Pastel gains a near-white frosted-paper surface, Cyber gains a faint HUD grid, LED gains dim top/bottom segments, and Dark Glass gains a quiet glass reflection. Aurora preserves its established sparse 52px field exactly and gains only low-opacity teal/blue veils.
- **Broadcast safety:** these are static CSS backgrounds, inset lines and shadows only—no animation, filter, image asset, request, price-feed change, card-height change, or event-effect change. List/Rotate and the compact Marquee all receive surface detail without changing their source dimensions. New copied URLs write `detail=rich`; parameter-less existing OBS URLs resolve to Basic so they cannot change appearance unexpectedly.
